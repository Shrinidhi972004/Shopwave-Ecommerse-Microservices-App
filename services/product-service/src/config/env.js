...
if (missing.length > 0) {
  console.error(
    `[product-service] Missing required environment variables: ${missing.join(', ')}\n` +
...
if (
  ...
  console.error('[product-service] Refusing to start in production with the example JWT_SECRET.');
  process.exit(1);
}
