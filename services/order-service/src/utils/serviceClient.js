...
/** Read a user's live cart from cart-service (used at checkout). */
async function fetchCart(userId, token) {
  return callService(`${config.cartServiceUrl}/api/cart/internal/${userId}`, { token });
}

/** Empty a user's cart after their order is committed. */
async function clearCart(userId, token) {
  return callService(`${config.cartServiceUrl}/api/cart/internal/${userId}`, {
    method: 'DELETE',
    token,
  });
}

/** Atomically reserve stock in product-service. Throws 409 if insufficient. */
async function decrementStock(productId, quantity, token) {
  return callService(`${config.productServiceUrl}/api/products/${productId}/decrement-stock`, {
    method: 'POST',
    body: { quantity },
    token,
  });
...
/** Compensating action for decrementStock — returns units to the catalogue. */
async function restock(productId, quantity, token) {
  return callService(`${config.productServiceUrl}/api/products/${productId}/restock`, {
    method: 'POST',
    body: { quantity },
    token,
  });
