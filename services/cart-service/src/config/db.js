...
const pool = new Pool({
  ...
  options: '-c search_path=cart_service,public',
});

pool.on('error', (err) => {
  console.error('[cart-service] Unexpected idle client error:', err.message);
});
...
async function query(text, params) {
  ...
  if (config.env === 'development') {
    ...
    if (ms > 200) console.warn(`[cart-service] slow query (${ms}ms): ${text.slice(0, 80)}`);
  }
  return result;
}
